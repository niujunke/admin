  ...
  <Row :gutter="10">
              ...
              <Col span="24">
                ...
                <Tooltip content="编辑" :transfer="true" style="float:right;">
                  <Button
                    ...
                    shape="circle"
                    icon="ios-create"
                    @click.navite="redactRole(item)"
                  ></Button>
                </Tooltip>
                <Tooltip
                  v-privilege="'disabled'"
                ...
                >
                  <Button
                    ...
                    shape="circle"
                    icon="ios-pause"
                    @click.navite="updateDisabled(item.id,1)"
                  ></Button>
                </Tooltip>
                <Tooltip
                  v-privilege="'disabled'"
                ...
                >
                  <Button
                    ...
                    shape="circle"
                    icon="ios-play"
                    @click.navite="updateDisabled(item.id,0)"
                  ></Button>
                </Tooltip>
              </Col>
            </Row>
    ...
    <Col :md="16" :lg="19">
      <Card class="warp-card">
        ...
        <!--成员管理-->
        <RoleList v-if="displayTab==2"></RoleList>
      </Card>
    </Col>
  </Row>
</template>
...
export default {
  ...
  methods: {
    ...
    loadList () {
      this.$Spin.show()
      groupApi.groupPageQuery().then(res => {
        if (res.success) {
    ...
    addloadRole (roleDesc, roleName) {
      ...
      groupApi.addRole(params).then(res => {
        if (res.success) {
          this.editRule(false)
          this.loadList()// 刷新
        }
    ...
    editRolefun (id, roleDesc, roleName) {
      ...
      groupApi.groupUpdate(params).then(res => {
        if (res.success) {
    ...
    // 编辑
    edittoRule () {
      this.editRolefun(this.ruleDetail.id, this.ruleDetail.detail, this.ruleDetail.name)
    },
    // 删除角色方法
    deleteRolefun (id) {
      this.$Spin.show()
      groupApi.groupDelete(id).then(res => {
        if (res.success) {
