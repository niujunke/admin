<template>
  ...
  <div>
    ...
    <Row type="flex" justify="end" class="page">
      ...
      <Col span="18" order="2" style="text-align: right;margin-top:20px;">
        <Page
          :total="total"
          :page-size="pageSize"
          :current="currentPage"
...
<script>
import DepartmentEmployeeSelector from "../department/components/department-employee-selector/DepartmentEmployeeSelector";
...
export default {
  ...
  methods: {
    ...
    removeEmployeeListFunc(param) {
      ...
      api.removeEmployeeList(param).then(res => {
        if (res.success) {
    ...
    selectChange(selection) {
      ...
      for (var i = 0; i < selection.length; i++) {
        this.removeId.push(selection[i].id);
      }
    ...
    remove: function(index) {
      ...
      Obj.employeeId = this.data[index].id;
      Obj.roleId = this.roleId;
      this.emoveEmployeeRoleFunc(Obj);
    },
    // 删除角色成员方法
    emoveEmployeeRoleFunc(param) {
      api.removeEmployeeRole(param).then(res => {
        if (res.success) {
    ...
    addEmployeeListRoleFunc(param) {
      ...
      api.addEmployeeListRole(param).then(res => {
        if (res.success) {
    ...
    // 获取角色id对应的成员列表方法
    async getlistEmployeeFunc(param) {
      try {
        let res = await api.getlistEmployee(param);
        if (res.success) {
    ...
    // 获取角色id对应的全部成员列表方法
    async getlistAllEmployeeFunc() {
      try {
        let res = await api.getAlllistEmployee(this.roleId);
        if (res.success) {
          let list = res.data;
          this.manageAdd = [];
          for (let i = 0; i < list.length; i++) {
            let Obj = {};
            Obj.manageName = list[i].actualName;
            Obj.manageId = list[i].id;
            this.manageAdd.push(Obj);
          }
        }
      } catch (e) {}
